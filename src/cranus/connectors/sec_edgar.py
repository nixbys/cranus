...
from cranus.connectors.registry import register_connector
...
class SecEdgarConnector(Connector):
    ...
    def __init__(self, config: dict | None = None):
        ...
        self._user_agent = self.config.get(
            "user_agent", "cranus-research-bot contact@example.com"
        )
    ...
    def parse(self, raw: RawObject) -> ParsedDocument:
        if raw.content_type == "application/pdf":
            from cranus.ingestion.extractors.pdf import extract_pdf

            extracted = extract_pdf(raw.content)
            text = extracted.text
        else:
            from cranus.ingestion.extractors.html import extract_html
